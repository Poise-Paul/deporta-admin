    ...
    const { paginatedData, totalPages } = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"].useMemo({
        "StaffManagementTable.useMemo": ()=>{
            ...
            let filtered = allStaffs.filter({
                "StaffManagementTable.useMemo.filtered": (staff)=>{
                    const searchStr = searchQuery.toLowerCase();
                    return staff.first_name?.toLowerCase().includes(searchStr) || staff.last_name?.toLowerCase().includes(searchStr);
                }
            }["StaffManagementTable.useMemo.filtered"]);
            // 2. Filter by Tab Status
            if (activeTab === "active") {
                filtered = filtered.filter({
                    "StaffManagementTable.useMemo": (s)=>s.status === "active"
                }["StaffManagementTable.useMemo"]);
            } else if (activeTab === "inactive") {
                filtered = filtered.filter({
                    "StaffManagementTable.useMemo": (s)=>s.status === "in-active"
                }["StaffManagementTable.useMemo"]);
            }
            // Role Logic
            if (roleFilter !== "all") {
                filtered = filtered.filter({
                    "StaffManagementTable.useMemo": (s)=>s.user_type.type_id.role.toLowerCase() === roleFilter.toLowerCase()
                }["StaffManagementTable.useMemo"]);
            }
...
//# sourceMappingURL=_8d9b7d86._.js.map